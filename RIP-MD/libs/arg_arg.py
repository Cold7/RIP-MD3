...
#/
#/////////////////////////////////////////////
def parse(pdbNames,folder, nproc, Nodes, distancia):
	...
	if len(pdbNames)>1:
		...
		pool.map(parallelArgArg,pdbNames.items(),chunksize=1)
		pool.close()
	else:
		...
		for i in range(len(ARGs)):
			...
			while j<len(ARGs):
				...
				j+=1
		pool=mp.Pool(processes=int(nproc)) #for multiprocessing
		edgesList=list(pool.map(singleParallelArgArg,parameters,chunksize=1))
		pool.close()
