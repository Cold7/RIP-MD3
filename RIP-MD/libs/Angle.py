...
def angle(v1,v2,v3): #v1 and v2 are two points in the space, v3 is a normal vector
	...
	#now we determine the scalar vector
	...
	angulo=np.arccos(scalar_prod / (m_vaux * m_v3)) # MODIFIED FORMULA: 1/COS() --> ARCCOS()
	return angulo * 57.2957795
...
def angle_polar_groups(v1,v2,v3,v4): #each v is a point in the space
	...
	#and now we will find the angle
	angulo=np.arccos(scalar_prod/(m_vaux*m_vaux2)) # MODIFIED FORMULA: 1/COS() --> ARCCOS()
	return angulo*57.2957795
...
def angle_twoVectors(v1,v2):
	...
	#and now we will find the angle
	angulo = np.arccos(scalar_prod / (m_v1 * m_v2)) # MODIFIED FORMULA: 1/COS() --> ARCCOS()
