...
def Pearson(pdbNames, output, Nodes, nproc, args):
	...
	pool.map(parallelCountInteraction,Nodes.items(),chunksize=1)
	...
	pool.map(matrixNodeTime,patterns,chunksize=1)
	pool.close()
	...
	#sometimes the diagonal lines have items equal to 0, to assess that this line is 1 we do the following
