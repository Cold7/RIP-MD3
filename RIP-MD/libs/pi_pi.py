...
def singleParallelPiPi(parameters):
	...
	if str(piCoords1)!=str(np.zeros(3)):
		centroid1 = np.sum(piCoords1, axis=0) / len(piCoords1)
		...
		if str(piCoords2)!=str(np.zeros(3)):
			centroid2 = np.sum(piCoords2, axis=0) / len(piCoords2)
			...
			if dist<=Dist:
				...
				if (node1 in nodes) and (node2 in nodes):
					...
					if (0 <= dihedral < 30) or (150 <= dihedral < 180):
...
def parse(pdbNames,folder, nproc, Nodes, distance):
	...
	if len(pdbNames)>1:
		...
		pool.map(parallelPiPi,pdbNames.items(),chunksize=1)
		pool.close()
	else:
		edges=open(outputFolder+"/RIP-MD_Results/Edges/"+list(pdbNames.items())[0][0]+".edges","a")
		edges.write("Pi-Pi\nSource Node\tTarget Node\tEdge Name\tDistance (center of rings)\tOrientation\n")
		...
		for i in range(len(piSystems)):
			...
			while j < len(piSystems):
				...
				j+=1
		pool=mp.Pool(processes=int(nproc)) #for multiprocessing
		edgesList=list(pool.map(singleParallelPiPi,parameters,chunksize=1))
		pool.close()
