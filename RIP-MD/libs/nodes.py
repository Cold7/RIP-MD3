...
##################################################
##
## function that writes all residues in format
...
###############################################
##
## function that extracts attribute data
##
###############################################
def parallelExtractAttr(parameters):	
	...
	#if we are not using the correct SO
	if flag!=0:
		reschainDict={}
		dsspDict={}
		for resid in u.atoms.residues:
	...
	for line in File:
		...
		if "#  RESIDUE A" in line:
			flag=1 #in this form, this line will not be read
	File.close()
...
###############################################
##
## Function that writes attributes of nodes
...
def secStructure(pdbNames, Nodes, outputFolder, nproc, RIPMDPath):
	...
	listOfAttr=list(pool.map(parallelExtractAttr,listToWork,chunksize=1)) #list of attrs will be a list of multiples list with format [ [frame] [ [id of node][sasa][sec struct 8 clases][sec struct 3 classes] ] ]
	pool.close()
