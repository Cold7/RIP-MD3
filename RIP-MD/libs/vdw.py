	...
	for i in range(len(sel)):
		...
		while j<len(sel):
			...
			try:
				...
				if leftRange <= euclidean_distance-(float(topology.nodes[atom1]["radius"]) + float(topology.nodes[atom2]["radius"])) <= rightRange:
					#now we know atoms with a certain distance between atoms radius, so then we will know the number of bonds between them
					#through shortestpath. If they have shortestpath we will asume vdw contact only of shortestpath >=vdwRange.
					#if atoms do not have shortest path (i.e. a double or more chains) we also asume a vdw contact
					...
					if  cantShortestPath>=int(vdwDist):
						...
						epsilon=math.sqrt(float(topology.nodes[atom1]["epsilon"])*float(topology.nodes[atom2]["epsilon"]))
						RminIJ=(float(topology.nodes[atom1]["radius"])+float(topology.nodes[atom2]["radius"]))
		...
		try:
			...
			if leftRange <= euclidean_distance-(float(topology.nodes[atom1]["radius"]) + float(topology.nodes[atom2]["radius"])) <= rightRange:
				...
				if  cantShortestPath>=int(vdwDist):
					...
					#listToReturn.append(str(nodes[str(graph.nodes[graphNodes[i]]["resname"])+":"+str(graph.nodes[graphNodes[i]]["chain"])+":"+str(graph.nodes[graphNodes[i]]["resid"])])+"\t"+str(nodes[str(graph.nodes[graphNodes[j]]["resname"])+":"+str(graph.nodes[graphNodes[j]]["chain"])+":"+str(graph.nodes[graphNodes[j]]["resid"])])+"\t"+str(graph.nodes[graphNodes[i]]["resname"])+":"+str(graph.nodes[graphNodes[i]]["chain"])+":"+str(graph.nodes[graphNodes[i]]["resid"])+":"+str(graph.nodes[graphNodes[i]]["atomName"])+"-(vdW)-"+str(graph.nodes[graphNodes[j]]["resname"])+":"+str(graph.nodes[graphNodes[j]]["chain"])+":"+str(graph.nodes[graphNodes[j]]["resid"])+":"+str(graph.nodes[graphNodes[j]]["atomName"])+"\t"+str(euclidean_distance)+"\n")
...
def parse(args, pdbNames, output, nproc, Nodes, vdw_distance, vdw_range,graphTopo):
	...
	if len(pdbNames)>1:
		...
		pool.map(vdwByFrame,pdbNames.items(),chunksize=1) #calling in parallel form the vdw function
		pool.close()
	else:
		edges=open(output+"/RIP-MD_Results/Edges/"+list(pdbNames.items())[0][0]+".edges","a")
		edges.write("vdW contacts\nSource Node\tTarget Node\tEdge Name\tDistance\tpotential (kJ/mole)\n")
		u=MDAnalysis.Universe(list(pdbNames.items())[0][1])
		sel=u.select_atoms("protein")
		atomList = list(range(len(sel)))
		pool=mp.Pool(processes=int(nproc)) #for multiprocessing
		edgesList=list(pool.map(lj,atomList,chunksize=1)) #calling in parallel form the function lj
		pool.close()
