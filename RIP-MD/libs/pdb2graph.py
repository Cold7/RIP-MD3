...
#"""""""""""""""""""""""""""""""""""""""""""""""""""""""
#"
#" main function that generates a graph based on
...
def convert(pdb, PSF, ff, parameter_file, output):
			...
			if flag==1:
				...
				for element in splitted:
					if element!="":
						auxList.append(element)
				try:
					graph.nodes[auxList[0]]['resname']=auxList[3]
				except:
					pass
				try:					
					graph.nodes[auxList[0]]['segid']=auxList[1]
				except:
					pass
				try:
					graph.nodes[auxList[0]]['resnum']=auxList[2]
				except:
					pass
				try:					
					graph.nodes[auxList[0]]['atomName']=auxList[4]
				except:
					pass
				try:	
					graph.nodes[auxList[0]]['atomType']=auxList[5]
				except:
					pass
				try:					
					graph.nodes[auxList[0]]['charge']=auxList[6]
				except:
					pass
				try:
					for item in atomData["resids"][auxList[3]].items():
						for item2 in item[1].items():
							if item2[0]==auxList[4]:
								graph.nodes[auxList[0]]['group']=item[0]
								graph.nodes[auxList[0]]['epsilon']=item2[1]["epsilon"]
								graph.nodes[auxList[0]]['radius']=item2[1]["radius"]
				except:
					pass
	...
	#################################################
			...
			for atom in residue.atoms:
				if res=="HIS":
					...
					if flag==2: #we found HD1 and HD2 so we have aHSP
						...
						res="HSD"						
				try:
					graph.nodes[str(atom.index+1)]['resname']=  str(atom.resname)
					graph.nodes[str(atom.index+1)]['segid']=    str(atom.segid)
					graph.nodes[str(atom.index+1)]['resnum']=   str(residue.resid)
					graph.nodes[str(atom.index+1)]['atomName']= str(dictAtomData[res+":"+ atom.name]["atomName"])
					graph.nodes[str(atom.index+1)]['atomType']= str(dictAtomData[res+":"+ atom.name]["atomType"])
					graph.nodes[str(atom.index+1)]['charge']=   str(dictAtomData[res+":"+ atom.name]["charge"])
					graph.nodes[str(atom.index+1)]['group']=    str(dictAtomData[res+":"+ atom.name]["group"])
					graph.nodes[str(atom.index+1)]['epsilon']=  str(dictAtomData[res+":"+ atom.name]["epsilon"])
					graph.nodes[str(atom.index+1)]['radius']=   str(dictAtomData[res+":"+ atom.name]["radius"])
					graph.nodes[str(atom.index+1)]['bonds']=""
					aux=""
					for i in range(len(dictAtomData[res+":"+ atom.name]["bonds"])):
						aux+=dictAtomData[res+":"+ atom.name]["bonds"][i]+","
					aux=aux[:-1]
					graph.nodes[str(atom.index+1)]['bonds']=str(aux)
				except:
					pass
		...
		nodeList2=list(graph.nodes())
		for node in nodeList:
			nodeAtomName=None
			try:
				nodeAtomName=graph.nodes[node]["atomName"]
				for node2 in nodeList2:
					bondedToNode2=None
					try:
						bondedToNode2=graph.nodes[node2]["bonds"]
						splitted=bondedToNode2.split(",")
						for atom in splitted:
							if atom==nodeAtomName:
								if graph.nodes[node]["segid"]==graph.nodes[node2]["segid"] and graph.nodes[node]["resname"]==graph.nodes[node2]["resname"] and graph.nodes[node]["resnum"]==graph.nodes[node2]["resnum"]:
