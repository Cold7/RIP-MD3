...
def singleParallelCationPi(parameters):
	...
	if piSystems[j].resname=="PHE" or piSystems[j].resname=="TYR":
		...
		try:
			...
			pass
	if aromaticCoords is not None:
		#getting the centroid
		centroid = np.sum(aromaticCoords, axis=0) / len(aromaticCoords)
		...
		if str(pointOfCharge)!=str(np.zeros(3)):
			...
			if str(normalVector)!=str(np.zeros(3)):
				#computing distance between the charge and the centroid
				...
				#		dihedral angle calculated between CA of the cation, the mass center of cation, closer atom and mass center of ring
						...
						if dihedral is not None:
							kind=""
							if (a5 > dihedral >= a6) or (a7 >= dihedral > a8):
								kind="planar"
							elif (a9 >= dihedral >= a10) or (a11 <= dihedral <= a12):
								kind="oblique"
							elif a13 < dihedral < a14:
								kind="orthogonal"			
...
def parse(pdbDict, PSF, folder, nproc, Nodes, dist, angle1, angle2, angle3, angle4):
	...
	a13=float(aux[0])
	a14=float(aux[1])
	#now we will call a function (with multiprocessing) to get cation-pi interactions
	parameters=[]
	#for MD
	if len(pdbDict)>1:
		...
		edges=list(pool.map(parallelCationPi,pdbDict.items(),chunksize=1))
		pool.close()
	#for PDB
	else:
		...
		pool=mp.Pool(processes=int(nproc)) #for multiprocessing
		edgesList=list(pool.map(singleParallelCationPi,parameters,chunksize=1))
		pool.close()
