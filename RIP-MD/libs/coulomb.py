...
#			print atom
	...
	for i in range(len(selection)):
		...
		for node in nodeList:
			try:
				group=int((topology.nodes[node]["group"].split("_"))[1])
				if group>maxGroup1:
					maxGroup1=group
			except:
				pass
		#if we found more than 1 group we create a list of list to have a local copy of which atoms form each group
			...
			for group in groupList1:
				for group2 in groupList2:
					...
					for node in group:
						...
						atomsFromGroup1.append(topology.nodes[node]["atomName"])
						# for the j- sumatory
						for node2 in group2:
							atomsFromGroup2.append(topology.nodes[node2]["atomName"])
							...
							if euclidean_distance<=Rrf:	#for the threshold
								...
								if int(excluded)>0:
									...
									if cantShortestPath>=int(excluded):
										...
										##
										#######################################################
										Vcoulomb+=(float(topology.nodes[node]["charge"])*float(topology.nodes[node2]["charge"]))/euclidean_distance
										flag=1 #to know that we entered here
									if RF:
										Vdd+=((-1*float(topology.nodes[node]["charge"]))*float(topology.nodes[node2]["charge"])*Crf*math.pow(euclidean_distance,2))/(2*math.pow(Rrf,3))
										Vdi+=((-1*float(topology.nodes[node]["charge"]))*float(topology.nodes[node2]["charge"])*(1-(0.5*Crf)))/Rrf
					...
					if flag!=0:
						...
						if Vcoulomb>=KbT:
							if (resNode1 in nodes) and (resNode2 in nodes):
								d=0
...
def parse(pdbNames, output, nproc, Nodes, Topology, Ecs, Erf, T, Krf, Rrf_threshold, coulomb_excluded, reactionField, selection):
	...
	if len(pdbNames)>1:
		...
		edgesList=list(pool.map(potentialByFrame,pdbNames.items(),chunksize=1)) #calling in parallel form the coulomb function
		pool.close()
	#for PDB
	else:
		#for a single PDB
		...
		resNumbers = list(range(len(sele)))

		edges=open(output+"/RIP-MD_Results/Edges/"+list(pdbNames.items())[0][0]+".edges","a")
		edges.write("Coulomb\nSource Node\tTarget Node\tEdge Name\tDistance\tCoulomb Potential (kJ/mole)\n")
		pool=mp.Pool(processes=int(nproc)) #for multiprocessing
		edgesList=list(pool.map(potential,resNumbers,chunksize=1)) #calling in parallel form the coulomb function
		pool.close()
