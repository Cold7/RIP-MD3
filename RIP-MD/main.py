...
default_backend=get_backend() # Saves default backend to restore it when needed. Needs to be done before importing other modules that then change it (like correlation).
...
if args.dc:
	try:
		cdUI(args.output,default_backend)
	except SystemExit:
		pass
	except:
		print("\nAn unknown error has occurred... check CDerror.log")
		errorLog = open("CDerror.log", 'a')
...
if args.dcd and args.psf and args.pdb:
	...
	if args.pdb:
		...
		if args.pdb:
			...
			if args.missing_atoms and not args.psf:
				...
				MA = missingAtoms.addAtoms(list(pdbNames.items())[0][1],args.pH,ripmdPath, args.selection, args.output)
				if MA =="good":
		...
		########################################################################
		if args.upf:
			print("Reading already extracted frames (Date: " + str(time.strftime("%y/%m/%d")) + " Time: " + str(time.strftime("%H:%M:%S")) + ")...")
			for entry in os.scandir(args.output + "/.temp_RIP-MD/"):
				if str(entry.name[:6]) == "frame_" and str(entry.name[-4:]) == ".pdb":
					pdbNames[entry.name[:-4]] = args.output + "/.temp_RIP-MD/" + entry.name
			pdbNames = dict(sorted(pdbNames.items()))
			print("Done (Date: " + str(time.strftime("%y/%m/%d")) + " Time: " + str(time.strftime("%H:%M:%S")) + ")")
		else:
	...
	# so it causes errors when it reads pdb
	print("Deleting CONECT lines from PDBs..." + str(time.strftime("%y/%m/%d")) + " Time: " + str(time.strftime("%H:%M:%S")) + ")...")
	...
	if args.pdb:
		...
		shutil.copy(lastPDB,args.output+"/RIP-MD_Results/representative.pdb")
	else:
	...
	if not args.keep_temp:
		try:
			...
			pass
			print("Temporary files folder saved in " + args.output + "/.temp_RIP-MD")
